...
# Try to import dstack-sdk (optional)
try:
    import dstack_sdk
    DSTACK_AVAILABLE = True
    logger = logging.getLogger(__name__)
    logger.info("dstack-sdk imported successfully")
except ImportError:
    DSTACK_AVAILABLE = False
    logger = logging.getLogger(__name__)
    logger.warning("dstack-sdk not available, falling back to hardcoded private keys")

# Configure logging
logging.basicConfig(level=logging.INFO)

class DistributedCounter:
    def __init__(self, instance_id: str, wallet_private_key: str = None, 
                 contract_address: str = None, rpc_url: str = "http://localhost:8545", 
                 port: int = 8080, use_dstack: bool = False, dstack_socket: str = None,
                 dstack_key_path: str = None, dstack_key_purpose: str = None):
        self.instance_id = instance_id
        self.port = port
        self.use_dstack = use_dstack and DSTACK_AVAILABLE
        ...
        if self.use_dstack:
            ...
            if not wallet_private_key:
                raise ValueError("wallet_private_key is required when not using dstack")
            self._init_hardcoded_wallet(wallet_private_key)
    ...
    def _init_dstack_wallet(self):
        ...
        try:
            ...
            if self.dstack_socket:
                self.dstack_client = dstack_sdk.DstackClient(self.dstack_socket)
            else:
                # Try to auto-detect socket
                if os.path.exists('./simulator/dstack.sock'):
                    self.dstack_client = dstack_sdk.DstackClient('./simulator/dstack.sock')
                else:
    ...
    def _init_hardcoded_wallet(self, wallet_private_key: str):
        """Initialize wallet using hardcoded private key (fallback)"""
        logger.info("Initializing wallet with hardcoded private key")
        self.wallet_private_key = wallet_private_key
        self.account = Account.from_key(wallet_private_key)
        self.wallet_address = self.account.address
        logger.info(f"Hardcoded wallet initialized: {self.wallet_address}")
